...
def save_user(user_claims):
    ...
    if not existing_user:
        ...
        # Clear role preference from session
        session.pop('role_preference', None)
    else:
...
def assign_user_role(user_claims):
    """
    Automatically assign user role based on criteria and login preference
    """
    email = user_claims.get('email', '').lower()
    
    # Check role preference from login flow
    role_preference = session.get('role_preference', 'student')
    ...
    # If user chose admin login, assign admin role
    if role_preference == 'admin':
        return 'admin'
